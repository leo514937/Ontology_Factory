...
TOOL_SPECS: list[dict[str, Any]] = [
    {
        "name": "run_command",
        "description": "在当前目标文档目录中执行命令行。支持只读检索命令，以及 CLI 形式的 Wiki/NER/relation/storage/ontology/dls 能力。可用示例：`pwd`、`ls`、`rg -n 关键词 .`、`cat 文件名`、`wikimg --root 工作区 search 关键词 --content`、`python -m ner.cli extract --input 文件 --query 关键词 --stdout`、`python -m entity_relation.cli extract --input 文件 --query 关键词 --stdout`、`python -m ontology_store.cli query --database storage/data/classification_store.sqlite3 --kind entities --query 关键词 --stdout`、`python -m ontology_core.cli search --database storage/data/classification_store.sqlite3 --query 关键词 --include-relations --stdout`、`python -m ontology_negotiator.cli classify --graph graph.json --config dls/config/ontology_negotiator.toml --stdout`。",
