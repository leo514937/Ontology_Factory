...
def build_topic_system_prompt() -> str:
    return (
        "你是工业知识百科策划助手。"
        "你需要从清洗后的工程文档中提炼适合建立 Wiki 主题页的候选页面。"
        "页面优先是主题页，而不是整篇文档页。"
...
def build_react_system_prompt() -> str:
    return (
        ...
        "首次使用任何工程 CLI 前，先读 README_CLI.md，并先执行 python tools/cli_baseline.py doctor。"
        "然后打开 tools/qagent_cli_prompts/ 下对应说明文件，再执行命令。"
        "如果任务是工程文档整理入库，优先直接执行 python tools/cli_baseline.py run pipeline -- --input <file> --mode engineering-doc。"
        "只有在 engineering-doc 模式经过实际验证后确认不适用时，才拆回 mm-denoise、ner、entity-relation、ontology-core、ontology-store、xiaogugit 分步执行。"
        "优先复用仓库现有 CLI，不要自行发明流程。"
...
TOOL_SPECS: list[dict[str, Any]] = [
    {
        ...
        "description": (
            "在当前目标文档目录中执行命令行。"
            "支持只读检索命令，以及 Wiki、NER、relation、storage、ontology、dls、pipeline、mm_denoise、xiaogugit、AFT 等 CLI。"
            "所有 CLI 都支持先用 --help 读取帮助。"
            "首次使用某个工程 CLI 前，先看 README_CLI.md，并执行 python tools/cli_baseline.py doctor。"
            "如果是工程文档处理任务，优先看 tools/qagent_cli_prompts/pipeline.md 和 engineering-doc-to-xiaogugit.md，"
            "并优先使用 python tools/cli_baseline.py run pipeline -- --input <file> --mode engineering-doc。"
            "可用示例：pwd、ls、rg -n 关键词 .、cat 文件名、wikimg --help、"
